...
export function SubmissionsTable({ data: initialData }: SubmissionsTableProps) {
  ...
  const handleDeleteConfirm = () => {
    if (submissionToDelete) {
      ...
      toast.success(`Submission "${submissionToDelete.title}" deleted successfully`);
  ...
  const handleFormSubmit = (submissionData: Partial<Submission>) => {
    if (submissionData.id) {
      setData(data.map((s) => (s.id === submissionData.id ? { ...s, ...submissionData } : s)));
    } else {
      const newSubmission: Submission = {
        ...
        leagueId: 1,
        leagueName: submissionData.leagueName || "",
        activityType: submissionData.activityType || "workout",
  ...
  const columns: ColumnDef<Submission>[] = [
    ...
    },
    {
      ...
      header: "League",
      cell: ({ row }) => (
        <span className="text-muted-foreground text-sm">{row.original.leagueName}</span>
      ),
    },
    {
